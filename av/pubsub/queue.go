...
type Queue struct {
	...
	lock                             *sync.RWMutex
	cond                             *sync.Cond
	curgopcount, maxgopcount, maxbuf int
...
func (self *Queue) SetMaxBufCount(n int) {
	self.lock.Lock()
	self.maxbuf = n
	self.lock.Unlock()
	return
...
func (self *Queue) WritePacket(pkt av.Packet) (err error) {
	...
	for self.buf.Count >= self.maxbuf {
		self.buf.Pop()
		/*if /*pkt.Idx == int8(self.videoidx) &&*/ /*pkt.IsKeyFrame {
			self.curgopcount--
		}
...
func (self *QueueCursor) ReadPackets() (pkts []av.Packet) {
	self.que.cond.L.Lock()
	buf := self.que.buf
	if !self.gotpos {
	...
	println(self.pos, buf.Tail)

	for self.pos < buf.Tail {
		pkts = append(pkts, buf.Get(self.pos))
		self.pos++
	}
